...
After long procrastination of learning in Kaggle, here I'm with my first piece of work. I forked this code from Megan Risdal which helped me greatly to understand lot of new conspts. I'm new to data science and machine learning. We'll try to learn as much from this forum.    

Thank you Megan Risdal

I'll first write the things I learn't from the Base script.

1. Great Visualizations
2. Feature extraction from existing data.
3. Missing Value Imputation using a different package "mice"
4. Random Forest Implementation.
5. Write a perfect markdown script.
...
## Building the model 
Before building the Random forest model, let's do Logistic Regression to find the Variable Significance.
...
summary(lr_model)
```
From Logistic Regression model, we gtot Pclass, Age, Title, FsizeD are significant. We'll build a Random forest model with these variables and compare with other models.
...
After getting the ranks of importance for the predictor variables from the above plot, Let's create few other models changing the variables including the Siginificance Varibles given by Logistic Regression. 
...
Let's plot the model error for all these models.
...
I chose the 2nd model for prediction because of the less OOB error estimate.
...
I did some experimentation on the base model and learnt quite few things in the process. 
Hurray.. I completed my first learning in Kaggle. Feel free to give feedback and suggestions. Thank you.
