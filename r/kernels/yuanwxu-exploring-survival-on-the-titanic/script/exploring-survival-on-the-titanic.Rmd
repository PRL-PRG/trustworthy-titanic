...
author: 'Megan L. Risdal/Yuanwei Xu'
date: '17 Sept 2016'
output:
  html_document:
...
**Test prediction with logistic regression. Section 1 to 3 remain the same as published by OP.** YX
...
Now we use the model to make preditions on the test data.
...
From the output of our model we see that some variables, such as `Pclass`, `Age` and `Title`, are more significant than others, but
correlations amongst the variables should also be noted, for example `Fsize` is by definition the sum of `SibSp` and `Parch`. Hence 
we restrict ourselves to a smaller subset of variables and refit the logistic regression model.
