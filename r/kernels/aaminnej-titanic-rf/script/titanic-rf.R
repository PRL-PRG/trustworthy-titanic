##################################################################################################
#                  KAGGLE - "Titanic: Machine Learning from Disaster" - R code
##################################################################################################
...
library(pastecs)
train <- read.csv('../input/train.csv', stringsAsFactors = F,header = TRUE)
test <- read.csv('../input/test.csv', stringsAsFactors = F,header=TRUE)
full  <- bind_rows(train, test) # Bind training & test data to extract most value before prediction
...
# Create 'Rare Title' value to summarise all "Upper Class" titles as there are too many of them
...
# Creating a 'Rare Title' variable entirely might be a good idea to avoid overfitting as the Title
# variable already contains some information about the Sex and Age
...
full$Fsize<-full$SibSp+full$Parch+1 # Create family size variable as follows:
# (Sibling/Spouse + Parents/Children + self)
# However Fsize variable does not account for passengers having both a sibling and a spouse meaning 
# Fsize will not be a constant number within that family. Attempt to rectify below.
...
# being largely in the First Class category. This won't be very useful to us so we will leave it.
...
mice_mod <- mice(full[, !names(full) %in% c('PassengerId','Name','Ticket','Cabin','FSize','Deck',
...
rf_model <- randomForest(factor(Survived) ~ Pclass + Sex + Age + FsizeD + Title + Child + Mother,
                         data = train)
################        MODEL ERROR       ####################
...
##############        VARIABLE IMPORTANCE       ################
# Taking a look at relative variable importance by plotting the mean decrease in Gini across all trees
...
solution <- data.frame(PassengerID = test$PassengerId, Survived = prediction) # Save solution
write.csv(solution, file = 'rf_mod_Solution.csv', row.names = F) # Write to csv
# Done!