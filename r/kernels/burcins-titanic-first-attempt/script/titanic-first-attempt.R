# Hello everyone, I am a self-learner and this is my first Kaggle script. 
#I got help from MOOCs mostly and started to learn apprx. 6 months ago. 
#I think the time is come to make my hands dirty, I checked some public Kernels for getting inspiration from them. 
#However, I paid particular attention not to use samples from other solutions. 
#Your helps would be appreciated with your comments, if I have mistakes or you have any suggestions.
...
#After spliting passenger names, I wonder if any Last Name matches btw passengers and is this related with passengers with any Siblings or Parents.  
length(unique(titanic2$Last_Name))
titanic2 %>% subset(SibSp>=1 | Parch >= 1)
# It doesn'nt seem they matched, then let's see if being with family helps to survive;
...
#now it is time to check and fill missing data,
...
# There is a clear correlation btw them, so we can use just one of them for prediction. 
#Now it seems better to split and check the data for using given Survived data. 
...
#But if your title is “Mr”, than unfortunately your change is so low no matter your Embark or Pclass. 
#In order to analyse “Mr” titled people, we may check total range first. 
...
#It shown from graph that, most of men with “Mr” title and btw age 20 to 40 have min chance to live.


#Lets try to analyze deeper this age range, 
...
#It seems from our graph that some of our lucky guys might not be in this class, because we filled our NA data in Age column with Median of Age data, which was 30. It may misguide us. 
#At last I also want to check, is there any Lucky guys on board with their family,
...
#After this funny stage, it may help us to predict passengers due to their Generation, for using this data, we can clarify ages btw 20-40 with separate our data to 3 parts due to passenger ages. But we should apply this to all data, rather than our train data, if we plan to use this for making predictions.
...
#SVM model gives the best result with unknown data(our test data, which splitted from given train data set). 
