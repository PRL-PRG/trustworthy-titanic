...
full$Mother <- ifelse(full$Sex == 'female' & full$Parch > 0 &
                        full$Age > 18 & full$Title != 'Miss', 1, 0)
full$nTicket[!is.na(full$Ticket)] <- sapply(full$Ticket[!is.na(full$Ticket)],
                                          function(e) table(full$Ticket)[[e]])
full$fTicket <- full$Ticket
full$fTicket[full$nTicket == 1] <- NA
full$fTicket[!full$fTicket %in% full$fTicket[1 : 891]] <- NA
full$nCabin[!is.na(full$Cabin)] <- sapply(full$Cabin[!is.na(full$Cabin)],
                                          function(e) table(full$Cabin)[[e]])
...
factor_var <- c("Survived", "Pclass", "Sex", "Embarked",
  "Title", "FsizeD", "Child", "Mother", "fTicket", "fCabin")
...
write.csv(test[, c(1, 2)], file = "Titanic_rpart_3.csv",
  row.names = FALSE)
