...
Set correct type of column and make simple imputation.
```{r, message=FALSE, warning=FALSE}
...
Make imputation with Age missing values.
```{r, message=FALSE, warning=FALSE}
...
full$Age <- imp$Age
```
After imputation mean and IQR look similar to original data.  


Calculate family size and make separate column which passenger is without family.  
```{r, message=FALSE, warning=FALSE}
...
tableplot(train[,-1])
```
In first look we can see prevalence Passangers from 1-st class, female, traveling with family and some title (Miss, Mrs) to be Survived.

Just discover this variable more detailed.
```{r}
...
```
We can see that female had chance to be survived in 3 time more than male.  
Also passengers from 1-st class and with family had more chance to be survived. 

##Fitting model
Create model fit with SVM method
```{r, message=FALSE, warning=FALSE}
...
The final values used for the model were __sigma__ = `r svmRadial$results[1,1]` and __C__ = `r svmRadial$results[1,2]`.  

Predict Survived passangers for the test set and write it.  
```{r, message=FALSE, warning=FALSE}
