...
train_trans <- combi[1:891,-1]
library(arules)
train_trans$Age <-discretize(train_trans$Age, method = "frequency", 3) # Continuous variables need to be binned / discretized
train_trans$Fare <- discretize(train_trans$Fare, method = "frequency", 3)

str(train_trans)
trans<- as(train_trans,"transactions")
itemFrequencyPlot(trans, topN=25,  cex.names=.5)

rules <- apriori(trans,parameter = list(minlen=2, supp=0.005, conf=0.8),appearance = list(rhs=c("Survived=0", "Survived=1"),default="lhs"),control = list(verbose=F))
