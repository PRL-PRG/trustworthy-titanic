...
Finally, before start, I want to mention that everyone is welcome leave comments and feedback. Thanks.
...
We won't touch again the for now. Respect to this point, I've seen other works in which the exploratory analysis includes the test data. Perhaps that is the most apropiated way in this case, but I prefer don't include it because it reduces generality of the model we will to perform.

Now, a first look at the dataset:
...
The distribution of the passengers are:
...
The ticket variable, at first glance, I can't see anything relevant so I'll remove it.
...
This variable represents the Passengers' Class which we're going to treat it as factor type:
...
In the cabin variable we find a lot of missing values, near 77% of the registers, most of these NA are related to Pclass 2 and 3. To deal with these we will to replace with _X_ and then analize the distribution.
...
To my surprise, it's true that the class is in some manner is correlate with the class, but I was expecting something more strongly. So, based on this fact, I will generate fare intervals because it could be helpful at the moment of training the model.
...
And the final relation with the survivors:
...
Interesting, while the 74% of women survived, only 19% of men survived.
...
These variable tell us about the family composition. I'm going to create a new variable, `family_size`, which indicate the size of the family. Also, to avoid correlation between these variable I'll remove `SibSp`. Then, I'm going to split them according with their sizes:
...
I will split the group related with the proportions of survivors in each inteval:
...
Some additional  relations I see in other works but I'm not sure about their usefulness.
...
Now, before start to make the model we have to apply the same transformation to the test set.
...
Now it's time to select the algorithm and the set configuration to train the model. Respect the algorithm, we're going to perform some kind of manual ensemble model.
...
Another version I want to probe is a combined model. To generate this new model I will use the _caretEnsemble_ package. Also, I will perform model for each gender:
...
Finally, it's time to generate the file with the predictions to upload. 
