...
tester  <- read.csv('../input//test.csv')
...
my_forest <- randomForest(as.factor(Survived) ~ Pclass + Sex + Age + SibSp + Parch + Fare + Embarked + Surname_Index +  Spouse + Siblings + parents + child + title_index  ,data = train, importance = TRUE)
