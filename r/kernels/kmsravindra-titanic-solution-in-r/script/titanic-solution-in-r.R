...
df = read.csv("../input/train.csv", header = T)
...
# factorize Embarked, Pclass and Sex variables 
...
# extract the list of titles from name field
library(qdapRegex)
...
# new feature - familysize ( note - this didn't had any impact on prediction if using svm )
...
# now impute the values for age. Used KNN imputation
df_imputed <- knnImputation(data = df,k=8) #KNN Imputation
#df_imputed <- centralImputation(data = df) #Central Imputation

# Standardizing the dataset ( z-score standardization for scaling the dataset)
num_attr = c("Age", "Fare", "SibSp", "Parch")
cat_attr = setdiff(colnames(df), num_attr)
...
# remove the non-numerical cols that are not required ( these are already factored as dummy variables 
# above and hence can be removed.
