...
confusionMatrix(data = factor(predclass_train), reference = factor(data.train$Survived))
...
confusionMatrix(data = factor(predclass_test), reference = data.test$Survived)
...
confusionMatrix(data = factor(predclass_all_train), reference = Titanic.train$Survived)
...
set.seed(24)
final_forest<- randomForest(Survived~Pclass+Sex+Age+SibSp+Parch+Embarked+IsChild+Is_Married+Fare, data = Titanic.train, ntree = 56, mtry = 1)
plot(final_forest)
