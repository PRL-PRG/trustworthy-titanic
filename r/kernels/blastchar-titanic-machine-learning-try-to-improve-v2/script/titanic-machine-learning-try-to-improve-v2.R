...
weights <- chi.squared(Survived~., DstAllClean[TrainSet, -which(names(DstAllClean) %in% c('Name', 'Ticket', 'Cabin', 'Surname', 'PassengerId'))])
# Select top five variables
subset <- cutoff.k(weights, 14)
# Print the final formula that can be used in classification
myFormula <- as.simple.formula(subset, "as.factor(Survived)")
# show me the formula to use 
print(myFormula)


# train the model
fit.rf <- cforest(myFormula, data = DstAllClean[TrainSet, c("Survived",subset)],
                  controls=cforest_unbiased(ntree=500, mtry=3))


# predict
pred <- predict(fit.rf, DstAllClean[TestSet, c("Survived",subset)], OOB=TRUE, type = "response")
