...
## Optimus Family Name, the transformer that gets the family name
...
type PP_FamNameTransformer <: ScikitLearnBase.BaseEstimator
end

@declare_hyperparameters(PP_FamNameTransformer, Symbol[])

function ScikitLearnBase.fit!(self::PP_FamNameTransformer, X::DataFrame, y=nothing)
    return self
end

function ScikitLearnBase.transform(self::PP_FamNameTransformer, X::DataFrame)
    @linq  X |>
        transform(CptName = map(s->match(r"^.*?,", s).match, :Name))
end
...
## Optimus Age*Class
type PP_AgeClassTransformer <: ScikitLearnBase.BaseEstimator
end

@declare_hyperparameters(PP_AgeClassTransformer, Symbol[])

ScikitLearnBase.fit!(self::PP_AgeClassTransformer, X::DataFrame, y=nothing) = return self

function ScikitLearnBase.transform(self::PP_AgeClassTransformer, X::DataFrame)
    @linq X |>
    transform(CptAgeClass = :CptAge + :Pclass )
end
