...
pred_log <- predict(logit,test3,type="response") 
summary(pred_log)
head(pred_log)

# Baseline Submission

write_to_file(pred_log, "logistic")
...
write_to_file(pred_test_ada$class, "Ada")
...
colnames(dummysex) <- c("Sexfemale", "Sexmale")
colnames(dummyemb) <- c("EmbarkedN","EmbarkedC","EmbarkedQ","EmbarkedS")
